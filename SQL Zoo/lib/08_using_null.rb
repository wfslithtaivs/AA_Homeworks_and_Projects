...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      COUNT(teachers), COUNT(COALESCE(mobile))
    FROM
      teachers
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, COUNT(COALESCE(teachers.name))
    FROM
      depts
    LEFT JOIN
      teachers ON depts.id = teachers.dept_id
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
    SELECT
      name,
      CASE WHEN dept_id BETWEEN 1 AND 2 THEN 'Sci' ELSE 'Art' END as suffix
    FROM
      teachers
  SQL
end
